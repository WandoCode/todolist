...
const todosReducer = createReducer(initialState, (builder) => {
    builder
        ...
        .addCase(switchItems, (state, action) => {
            ...
            const listB = getTodosListName(action.payload.listB)
            const listA = getTodosListName(action.payload.listA)

            const itemA = state[listA][indexA]

            itemA.status = action.payload.listB

            state[listA].splice(indexA, 1)
            state[listB].splice(indexB, 0, itemA)
        })
